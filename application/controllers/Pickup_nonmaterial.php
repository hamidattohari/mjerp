...
class Pickup_nonmaterial extends MY_Controller {
	...
	public function details($woid, $pid)
	{
		...
		$data['title'] = "ERP | Pickup Non Materials";
		$data['page_title'] = "Pickup Non Materials <strong>".$pname->name."</strong>";
		$data['table_title'] = "List Item";		
		$data['breadcumb']  = array("Production", "Pickup Non Materials");
		$data['page_view']  = "production/pickupnm";		
		$data['js_asset']   = "pickupnm";	
	...
	function jsgrid_functions($id=-1){
		switch($_SERVER["REQUEST_METHOD"]) {
			...
			$this->upi->material_usage_delete();
	...
	public function add_detail(){
		$stock_status = $this->upi->check_material_stock($this->input->post('item_id'), $this->input->post('qty'));
		if($stock_status['status']){
			...
			if(isset($id)){
				$status = $this->upi->material_usage_change($id, $this->input->post('item_id'), $detail, "out");
			}
			echo json_encode(array('id'=> $id));
		}else{
	...
	public function update_detail(){
		...
		if(isset($status)){
			$status = $this->upi->material_usage_change($this->input->post('details_id'), $this->input->post('item_id'), $detail, "out");
		}
		echo json_encode(array('id'=> $status));
	}
