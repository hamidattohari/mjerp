...
$(document).ready(function() {
	...
	table = $('#datatable').DataTable({
        ...
        ajax: {
            url: site_url+'processes/view_data',
            type: "POST",
            dataSrc : 'data',
            data: function ( d ) {
...
function save_data(){
	var url;
	if(action == "Add"){
		url = site_url+"processes/add";
	}else{
		url = site_url+"processes/update";
	}
...
function edit(id){
	...
	$.ajax({
			url : site_url+"processes/get_by_id/"+id,
			type: "GET",
			dataType: "JSON",
			success: function(data)
			{
...
function remove(id){
	swal({
		...
		function(){
			$.ajax({
				url : site_url+"processes/delete/"+id,
				type: "GET",
				dataType: "JSON",
				success: function(data)
				{
